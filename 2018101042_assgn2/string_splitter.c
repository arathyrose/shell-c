...
int is_redirection_or_piping_char(char s) // Checks if the given character is a redirection or piping symbol and returns 1
{
    if (s == '<' || s == '>' || s == '|')
        return 1;
    else
...
char **string_splitter(char *buffer, char sep, int redirection) //splits the buffer based on the separator
{
    ...
    if (strcmp(buffer, "") == 0)
    {
        ...
        result[0] = (char *)memalloc_str(MAX_BUFFER_LENGTH * sizeof(char));
        result[0][0] = '\0';
        return result;
    }
    if (debug_string_splitter)
    ...
    for (int i = 0; i < strlen(buffer); i++)
    {
        if (buffer[i] == ' ')
            continue;
        result[rowno] = (char *)memalloc_str(MAX_BUFFER_LENGTH * sizeof(char));
        int j = 0;
        while (buffer[i] != '\0' && !is_sep(buffer[i], sep))
        {
            if (redirection == 1 && is_redirection_or_piping_char(buffer[i]))
            {
                if (j != 0) //it was in the middle of something
                {
                    result[rowno][j] = '\0';
                    if (debug_string_splitter)
                    {
                        char *s = memalloc_str(100);
                ...
                while (is_redirection_or_piping_char(buffer[i]))
