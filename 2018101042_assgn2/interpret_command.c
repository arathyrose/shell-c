...
void interpret_command(char *cmd, int forcebg)
{
    ...
    for (int i = 0; !is_emptystr(cmds[i]); i++)
    {
        ctrlc = 0;
        ctrlz = 0;
        ...
        if (is_emptystr(args[0]))                       // that is, no command has been given
            continue;
        if (isvalid_redirection(args) == 0) // if the redirection is illegal, better escape
            continue;
        if (execute_builtin_no_output(args) == 1) // if it is a builtin command that has no output and hence need not be forked and messed up, execute
            continue;
        if (pipe_handler(cmds[i]) != -1) // if contain any pipes, go separate
            continue;
        if (forcebg == 1 || strcmp(args[0], "cronjob") == 0) // since cronjob should be done in the background, no matter what happens
            isbg = 1;
        else
        ...
        if (pid == 0) // child process
        {
            if (isbg)
                setpgid(0, 0);
            set_ip_op_redirection(args);             // dup and set up stuff in case of input output redirection
        ...
        else
        {
            if (!isbg)
            {
                ...
                int statusssssss;
                waitpid(pid, &statusssssss, WUNTRACED); // wait, till it gets over
