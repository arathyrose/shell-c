/**
 * This function displays the shell prompt using colors
...
char *display_shell_prompt()
{
    ...
    char *username = memalloc_str(MAX_USERNAME_SIZE);
    char *system_name = memalloc_str(MAX_SYSTEM_NAME);
    char *current_directory = memalloc_str(MAX_CURRENT_DIR);
    ...
    char *buffer = memalloc_str(MAX_CURRENT_DIR + MAX_SYSTEM_NAME + MAX_USERNAME_SIZE + 38);
